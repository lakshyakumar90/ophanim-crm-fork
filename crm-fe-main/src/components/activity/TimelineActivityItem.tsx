...
export function TimelineActivityItem({
  ...
  const colorClass = actionColors[activity.activity_type] || "bg-muted text-muted-foreground ring-border";
  ...
  return (
    <div key={activity.id} className="relative flex items-start gap-4 group">
      ...
      <div className="flex flex-col items-center gap-1 w-[50px] sm:w-[60px] shrink-0 pt-0.5">
        <span className="text-[11px] sm:text-xs font-semibold text-muted-foreground">{formatIST(activity.created_at, "HH:mm")}</span>
        <div className={cn("relative z-10 rounded-full p-1.5 ring-4 ring-background bg-card", colorClass)}>
          <ActionIcon className="h-3.5 w-3.5" />
        </div>
      </div>

      {/* Content */}
      <div className="flex-1 min-w-0 pb-6 border-b border-border/50 group-last:border-transparent transition-colors">
        <div className="flex flex-col sm:flex-row sm:items-baseline sm:justify-between gap-1">
          <p className="text-sm text-muted-foreground leading-snug">
            <span className="font-bold text-foreground">{activity.user?.full_name || "System"}</span>{" "}
            <span className="font-medium">{formatActivityDescription(activity)}</span>{" "}
            {entityName !== "Activity" && (
              <span className="font-semibold text-[10px] tracking-widest uppercase bg-muted text-foreground px-1.5 py-0.5 rounded ml-1">
                {entityName}
              </span>
            )}
          </p>
        ...
        {commentPreview && (
          <p className="mt-2 text-sm italic text-muted-foreground border-l-2 border-border pl-3 py-1 bg-muted/50 rounded-r-md">
            &quot;{commentPreview}&quot;
          </p>
        )}
